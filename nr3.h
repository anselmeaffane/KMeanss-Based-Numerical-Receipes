...
public:
	...
	void add (int row, int col, const T & value) ;//Ajouté par Eugène Capo-Chichi pour charger la matrice (voir l-404)
	void print ();//Ajouté par Eugène Capo-Chichi pour faire de l'affichage (voir l 416)
	~NRmatrix();
};
...
//Ajouté par Eugène Capo-Chichi
template <class T>
void NRmatrix<T>::add (int row, int col, const T & value) {
	if (row  <= nn || col <= mm) {
		v[row][col] = value ;
	} else {
		throw("NRmatrix not limit | out of bounds");
	}
